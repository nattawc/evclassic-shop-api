              ...
              <div class="summary entry-summary position-relative">
				...
				<?php for($x = 0; $x < count($d->data['pProps']); $x++) { ?>
					<?php $row = $d->data['pProps'][$x]; ?>
						<?php // print_r($row); ?>
                      <tr>
                        <td class="align-middle text-2 px-0 py-2" style="min-width: 80px"><?php echo $row['prop']; ?>:</td>
                        <td class="px-0 py-2">
								<?php // print_r($row); ?>
							<?php for($i = 0; $i < count($row['value']); $i++) { ?>
								<?php if(isset($row['value'][$i]['imageUrl'])) { ?>
							 <img src="<?php echo $row['value'][$i]['imageUrl']; ?>" width="45" id="opt_<?php echo $x; ?>_<?php echo $row['prop']; ?>_<?php echo $row['value'][$i]['name']; ?>" class="btn btn-default btn-xs mb-1" style="padding: 0" />
								<?php } else { ?>
								<button id="opt_<?php echo $x; ?>_<?php echo $row['prop']; ?>_<?php echo $row['value'][$i]['name']; ?>" class="btn btn-default btn-xs mb-1" type="button"><?php echo $row['value'][$i]['name']; ?></button>
								<?php } ?>
							<?php } ?>
				...
				<?php } ?>
				  ...
				  <?php // print_r($d->data['pSkuParam']['skuRangePrices']); ?>
                  <hr />
              </div>
            </div>
          </div>
